...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_outputs, encoder_hidden, input_mask,
        ...
        if(target_tensors is not None): # during training
            embedded_input = self.embedding(target_tensors)
            ...
            for i in range(embedded_input.size(1)):
                ...
                if torch.isnan(decoder_hidden[0][0]).any():
                    print("NAN input_i:", input_i[0][0])
                ...
                #print("combined_output:", combined_output[0][0])
                decoder_outputs_list.append(combined_output)
        else: # during inference
            input_i = torch.tensor([self.bos_token_id] * encoder_outputs.size(0)).unsqueeze(1).to(device)
            input_i = self.embedding(input_i)
            for i in range(encoder_outputs.size(1)):
                decoder_output, decoder_hidden = self.gru(input_i, decoder_hidden)
                #print("decoder_output:", decoder_output.size(), "decoder_hidden:", decoder_hidden.size())
                query = decoder_hidden.view(-1, decoder_hidden.size(-1))
                context, _ = self.attention(query, values, input_mask)
                combined_output = torch.cat((decoder_hidden.permute(1, 0, 2), context.unsqueeze(1)), dim=2)
                decoder_outputs_list.append(combined_output)
                input_i = self.out(combined_output).argmax(dim=-1)
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, inputs, input_mask, targets=None, device=0):
